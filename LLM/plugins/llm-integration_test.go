...
// THÊM MỚI: Test kiểm tra lỗi thiếu API Key của Gemini
func TestBuildExecuteModelFailValidationGemini(t *testing.T) {
	llmHoneypot := LLMHoneypot{
		Histories:    make([]Message, 0),
		GoogleAPIKey: "", // Để trống key
		Protocol:     tracer.SSH,
		Model:        "gemini-1.0-pro",
		Provider:     Gemini,
	}

	geminiVirtualTerminal := InitLLMHoneypot(llmHoneypot)

	_, err := geminiVirtualTerminal.ExecuteModel("test")

	assert.Equal(t, "googleAPIKey is empty", err.Error())
}
...
func TestBuildExecuteModelOpenAISecretKeyFromEnv(t *testing.T) {
	...
	os.Unsetenv("OPEN_AI_SECRET_KEY") // Dọn dẹp biến môi trường
}
...
func TestBuildExecuteModelFailValidationModelType(t *testing.T) {
	...
	llmHoneypot := LLMHoneypot{
		...
		Protocol:  tracer.SSH,
		Model:     "llama3",
		Provider:  5, // Giả lập một provider không hợp lệ
	}
...
// THÊM MỚI: Test kiểm tra Gemini hoạt động thành công
func TestBuildExecuteModelSSHWithResultsGemini(t *testing.T) {
	client := resty.New()
	httpmock.ActivateNonDefault(client.GetClient())
	defer httpmock.DeactivateAndReset()
	...
	geminiMockURL := fmt.Sprintf(geminiEndpoint, modelName) // Xây dựng URL mock chính xác

	httpmock.RegisterResponder("POST", geminiMockURL,
		func(req *http.Request) (*http.Response, error) {
			resp, err := httpmock.NewJsonResponse(200, &GeminiResponse{
				Candidates: []struct {
	...
	llmHoneypot := LLMHoneypot{
		Histories:    make([]Message, 0),
		GoogleAPIKey: "dummy-gemini-key", // Dùng key giả vì đã mock request
		Protocol:     tracer.SSH,
		Model:        modelName,
		Provider:     Gemini,
	}
	...
	geminiVirtualTerminal := InitLLMHoneypot(llmHoneypot)
	geminiVirtualTerminal.client = client

	//When
	str, err := geminiVirtualTerminal.ExecuteModel("ls")

	//Then
	assert.Nil(t, err)
	assert.Equal(t, "gemini-response.txt", str)
}
...
// SỬA ĐỔI: Thêm case "gemini" vào bài test
func TestFromString(t *testing.T) {
	...
	model, err = FromStringToLLMProvider("gemini")
	assert.Nil(t, err)
	assert.Equal(t, Gemini, model)
